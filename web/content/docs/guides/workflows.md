...
| `coordinate` | an ad-hoc decomposition, the standard fan-out → synthesize → verify shape |
| **`workflow` (script)** | orchestration the **model writes**, when the shape isn't a plain fan-out |
| **`workflow` (name)** | a **named template** you wrote, re-runnable and resumable |

## The eleven things you can do

1. [Let the model write the orchestration](#model-authored-scripts) — loops, per-item pipelines, budget-aware fan-out
2. [Run a workflow by name](#running-one) — `/workflows run review target=…`
3. [Multi-phase execution](#the-definition-format) — phases run in order, each rolling up its own status
4. [Parallel fan-out and per-item pipelines](#phase-modes)
5. [Background it](#backgrounding) — returns a run id + job id immediately
6. [Disposable expert subagents](#agent-types) — built-in personas or your own Markdown ones
7. [Watch it live](#the-viewer) — phase rail, per-agent status, tokens, cost, timing
8. [Drill into one agent](#drilling-in) — its prompt, activity, result, error
9. [Watch several at once](#the-viewer) — multiple concurrent runs is the normal case
10. [Control it](#controls) — stop, pause/resume, cancel, skip, retry, save
11. [Inspect and resume history](#history-and-resume) — long after the session ended

---

## Model-authored scripts

A named definition is data: phases, agents, prompts. That covers most
orchestration — but not *loop until two consecutive rounds find nothing new*,
or *scale the fan-out to whatever budget is left*. For those the model writes
the orchestration itself, in a small Python subset:

```python
meta = {"name": "review-diff",
        "phases": [{"title": "Review"}, {"title": "Verify"}]}

phase("Review")
reviews = await parallel([
    (lambda d=d: agent(f"Review {args['target']} for {d} problems", label=d))
...
The hooks are the engine's own API:

| | |
|---|---|
| `await agent(prompt, label=, phase=, agent_type=, model=, schema=)` | one child agent → its final text |
| `await parallel([thunks])` | barrier — waits for all |
| `await pipeline(items, *stages)` | per-item chains, **no barrier** between stages |
| `phase(title)` · `log(msg)` | the rail and the run log |
| `args` · `budget.total/spent()/remaining()` · `json` | inputs and spend |

Thunks are zero-arg lambdas that *call* `agent`, and must bind the loop
variable by default argument (`lambda d=d: …`) — the usual Python late-binding
trap, and the one mistake that makes a fan-out run the same prompt N times.

**Prefer `pipeline` over back-to-back `parallel`.** A pipeline lets item B
enter stage 2 while item A is still in stage 1; a barrier makes every fast item
wait for the slowest. Only put a barrier between stages when a stage genuinely
needs *all* prior results at once — deduping across the whole set, an early
exit on a total count, or a prompt that compares findings to each other.

### What a script may do

Loops, conditionals, comprehensions, f-strings, `try`/`except`, `def`,
`lambda`. **Not** imports, file or network access, or attributes starting with
`_`. Anything outside orchestration is an agent's job, where the permission
system applies — and the agents a script spawns inherit the parent's tools,
permissions and budget unchanged, so writing the orchestration never widens
what runs inside it.

This is a guardrail, not a security boundary: the script's author is the
session's own model, which in most configurations can already run shell
commands. It keeps a confused or injected script inside its job.

A script is validated before anything spawns, so a bad one costs nothing and
comes back with the line number from your own source.
...
* [Sub-agents](/docs/guides/sub-agents) — the personas a workflow's phases run
* [Budget & limits](/docs/guides/budget) — capping what a fan-out can spend
* [Permissions](/docs/guides/permissions) — the gate children inherit
