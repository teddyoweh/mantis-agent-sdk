...
- **Activity · last 26 weeks** — the message trace, the weekday×hour
  punchcard, the tool spectrum and the projects ledger (with estimated
  tokens per project).
...
runtimes — the same OSS id costs differently on Groq and Together),
capability badges (tools / effort / thinks), and a one-click *use →*.
Filter chips: all, ready to use, needs a key, free / local.
...
guide. The **Self-host / custom endpoint** form lives here too: base URL,
model id, optional key, Connect.

### Deploy

Bring your own GPU cloud. The page is the hub for the
[deploy feature](deploy.md) (the same thing `mantis-agent deploy` and the
...
- **GPU providers** — one card per adapter (RunPod, Hugging Face Inference
  ...
  form generated from the adapter's own credential fields — secret fields are
  password inputs, each with its help text and a link to the provider's
...
| `/api/analytics`, `/api/projects`, `/api/sessions`, `/api/skills`, `/api/mcp`, `/api/config` | the rest of the pages |

Deploy endpoints (`POST` bodies are JSON; every `POST` needs the token):

| Endpoint | Does |
|---|---|
| `GET /api/deploy/providers` | every deploy adapter: configured, credential fields, engines, console URL, scale-to-zero / public flags, last validation result |
...
- **Deploy credentials go to your user settings env.** Saving a provider key
  ...
  under `~/.mantis-agent` (the same place `/enable` puts provider keys,
  written `0600`) and exports it into the running process. The value is never
  sent back to the page — not even masked — only the names of the variables
  that were saved.
