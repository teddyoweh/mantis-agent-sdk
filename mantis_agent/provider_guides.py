...
DEPLOY_GUIDES: dict[str, dict[str, Any]] = {
    ...
    "fireworks-dedicated": {
        "name": "Fireworks",
        "intro": "Dedicated GPUs for a model from the Fireworks library, behind the same API as their serverless models — the key you already use is the whole setup.",
        "env_var": "FIREWORKS_API_KEY",
        "env_vars": [
            {"name": "FIREWORKS_API_KEY", "required": True,
             "note": "Settings → API Keys; the same key serverless inference uses"},
            {"name": "FIREWORKS_ACCOUNT_ID", "required": False,
             "note": "only when the key can see more than one account"},
        ],
        "keys_url": "https://app.fireworks.ai/settings/users/api-keys",
        "signup_url": "https://app.fireworks.ai/signup",
        "pricing_url": "https://fireworks.ai/pricing",
        "docs_url": "https://docs.fireworks.ai/guides/ondemand-deployments",
        "steps": [
            "Sign up at app.fireworks.ai (Google, GitHub or email)",
            "Open Settings → API Keys and click 'Create API key'",
            "Copy the key (shown once)",
            "Add a payment method under Billing — on-demand GPUs are billed per GPU-second",
            "mantis-agent deploy creds fireworks-dedicated --set FIREWORKS_API_KEY=…",
        ],
        "key_hint": "Starts with fw_; the same key as serverless inference.",
        "free_note": "New accounts get starter credit; dedicated GPUs need a card on file.",
        "cost_note": "H100 $8/h, H200 $8/h, B200 $13/h, B300 $15/h per GPU, per second; $0 while scaled to zero.",
    },
    "vastai": {
        "name": "Vast.ai",
